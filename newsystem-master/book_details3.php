...
// Check if user is logged in and fetch buyer ID
if (isset($_SESSION['id'])) {
    $buyerid = $_SESSION['id']; // User's ID stored in the session
} else {
    echo "You must log in to view this page.";
    exit();
}

$searchKeyword = ''; // Initialize search variable
...
// Fetch cart details for the current buyer
      ...
      <div class="col-md-7">
        ...
        <p class="book-price h4 font-weight-bold mb-4" style="color: #000;">RM <?= number_format($book['price'], 2) ?></>
          ...
          <div class="btn-container mt-4">
            <a href="checkout.php?idbook=<?= $book['idbook'] ?>" class="btn btn-primary btn-sm px-4 mr-2"><i class="fas fa-shopping-cart"></i> Check Out</a>
            <a href="my_cart.php" class="btn btn-secondary btn-sm px-4"><i class="fas fa-arrow-left"></i> Back to Carts</a>
          </div>
        </div>
      </div>
    </div>
